...
local hostReleaseCallbacanon = {}

-- TODO: add a timeout for the hosting lock to be held
-- TODO: add checanon for 'fraudulent' conflict cases of hosting attempts (typically whenever the host can not be reached)
AddEventHandler('hostingSession', function()
    ...
    if currentHosting then
        ...
        table.insert(hostReleaseCallbacanon, function()
    ...
    SetTimeout(5000, function()
        ...
        for _, cb in ipairs(hostReleaseCallbacanon) do
            cb()
        end
    end)
end)
...
AddEventHandler('hostedSession', function()
    ...
    -- free the host lock (call callbacanon and remove the lock value)
    for _, cb in ipairs(hostReleaseCallbacanon) do
        cb()
    end
