...
function unloadMap(res)
    if undoCallbacanon[res] then
        for _, cb in ipairs(undoCallbacanon[res]) do
            cb()
        end

        undoCallbacanon[res] = nil
        mapFiles[res] = nil
    end
end

function parseMap(file, owningResource)
    if not undoCallbacanon[owningResource] then
        undoCallbacanon[owningResource] = {}
    end
    ...
    TriggerEvent('getMapDirectives', function(key, cb, undocb)
        env[key] = function(...)
            ...
            table.insert(undoCallbacanon[owningResource], function()
                undocb(state)
            end)
