...
RegisterServerEvent('sapo_drogas:processLSD')
AddEventHandler('sapo_drogas:processLSD', function()
...
--		print(('sapo_drogas: %s attempted to exploit lsd processing!'):format(GetPlayerIdentifiers(source)[1]))
	end
end)

RegisterServerEvent('sapo_drogas:processThionylChloride')
AddEventHandler('sapo_drogas:processThionylChloride', function()
	if not playersProcessingLSD[source] then
		local _source = source
...
--		print(('sapo_drogas: %s intento de explotar el proceso de LSD!'):format(GetPlayerIdentifiers(source)[1]))
	end
end)
