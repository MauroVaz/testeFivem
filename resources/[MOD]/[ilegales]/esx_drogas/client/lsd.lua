	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.lsdProcessing.coords, true) < 2 then
			if not isProcessing then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para procesar LSD", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function Processlsd()
	...
	TriggerServerEvent('sapo_drogas:processLSD')
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.thionylchlorideProcessing.coords, true) < 2 then
			if not isProcessing then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para procesar cloruro de tionilo", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function Processthionylchloride()
	...
	TriggerServerEvent('sapo_drogas:processThionylChloride')
