	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para procesar cannabis", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function ProcessWeed()
	...
	TriggerServerEvent('sapo_drogas:processCannabis')
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para cosechar la planta de cannbis", vector3(coords.x, coords.y, coords.z + 1))
			end

			s = 30
			if IsControlJustReleased(0, 35) and not isPickingUp then
				if isAllowedDrug('weed') then
...
function PickUpWeed(playerPed, coords, nearbyObject, nearbyID)
	...
	ESX.TriggerServerCallback('sapo_drogas:canPickUp', function(canPickUp)
		...
		if canPickUp then
			...
			TriggerServerEvent('sapo_drogas:pickedUpCannabis')
			Citizen.Wait(5000)
