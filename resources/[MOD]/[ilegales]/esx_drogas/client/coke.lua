	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.CokeProcessing.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para procesar cocaína", vector3(coords.x, coords.y, coords.z + 1))
			end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para cosechar la planta de coca", vector3(coords.x, coords.y, coords.z + 1))
			end
