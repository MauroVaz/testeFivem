	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.HeroinProcessing.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowFloatingHelpNotification("Pulsa [E] procesar la heroína", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function ProcessHeroin()
	...
	TriggerServerEvent('sapo_drogas:processPoppyResin')
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa [E] cosechar el cogollo de la amapola", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function PickUpPoppy(playerPed, coords, nearbyObject, nearbyID)
	...
	ESX.TriggerServerCallback('sapo_drogas:canPickUp', function(canPickUp)
		...
		if canPickUp then
			...
			TriggerServerEvent('sapo_drogas:pickedUpPoppy')
			Citizen.Wait(5000)
