	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa [E] para recoger el ácido sulfúrico del bote", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function PickUpSulfuricAcid(playerPed, coords, nearbyObject, nearbyID)
	...
	ESX.TriggerServerCallback('sapo_drogas:canPickUp', function(canPickUp)
		...
		if canPickUp then
			...
			TriggerServerEvent('sapo_drogas:pickedUpSulfuricAcid')
			Citizen.Wait(5000)
