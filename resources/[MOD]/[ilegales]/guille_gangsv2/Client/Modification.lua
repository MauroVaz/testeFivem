    ...
    Citizen.CreateThread(function()
        ...
        while true do
            if WarMenu.Begin('opt') then
                ...
                WarMenu.MenuButton('Delete ' ..gang.. " [DANGEROUS]", 'delete')
                WarMenu.End()
                ...
                for k, v in pairs(createdGangs) do
                    if k == gang then
                        for key, val in pairs(v.shop) do
                            if WarMenu.Button("Item name: " ..val.name.. " - " ..val.price.. "$") then
            ...
            elseif WarMenu.Begin("addshop") then
                local pressed, type = WarMenu.InputButton('Add item type (item or weapon): '..itemType, nil, _inputText)
                if pressed then
                    if type then
                ...
                local pressed, name = WarMenu.InputButton('Add name: '..itemName, nil, _inputText)
                if pressed then
                    if name then
                        itemName = name
                    end
                end
                local pressed, price = WarMenu.InputButton('Add price: '..itemPrice, nil, _inputText)
                if pressed then
                    if price then
                        itemPrice = price
                    end
                end
                local pressed, label = WarMenu.InputButton('Add label: '..itemLabel, nil, _inputText)
                if pressed then
                    if label then
                        itemLabel = label
                    end
            ...
            elseif WarMenu.Begin("delete") then
                if WarMenu.Button("Are you sure? [This can not be reversed]") then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if typePoints[start] ~= "Get Vehicles" then
                ...
                DeleteVehicle(veh)
                ShowHelpNotification('Press ~INPUT_CONTEXT~ to add the point. Use ~INPUT_CELLPHONE_RIGHT~ and ~INPUT_CELLPHONE_LEFT~ to pass throught points, adding ' ..typePoints[start].. ". Press ~INPUT_FRONTEND_RRIGHT~ to stop.")
            else
                ShowHelpNotification('Press ~INPUT_CONTEXT~ to add the point. Use ~INPUT_CELLPHONE_LEFT~ and ~INPUT_CELLPHONE_RIGHT~ to pass throught points, adding ' ..typePoints[start].. ". Keep pressed ~INPUT_AIM~ or ~INPUT_MAP_POI~ to modify the heading. Press ~INPUT_FRONTEND_RRIGHT~ to stop.")
            end
            ...
            if IsControlJustPressed(0, 194) then
                ...
                Citizen.Wait(250)
                openSelectMenu(gang)
                notify("Remember to confirm the points in points, if you do not confirm them, they will not be saved")
                editingPoints = false
                break
            end
        end
