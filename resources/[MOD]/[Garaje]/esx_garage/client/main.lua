		...
		for garage, garageData in pairs(Config.Garages) do
			for action, actionData in pairs(garageData["positions"]) do
				...
				if dstCheck <= 10.0 then
					...
					if draw then
						local markerSize = action == "vehicle" and 4.0 or 1.4
						if dstCheck <= markerSize - 0.1 then
							local usable = not DoesCamExist(cachedData["cam"])
							-- local playerPed = PlayerPedId()
							-- local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
							-- esx.ShowFloatingHelpNotification("Pulsa [E] para acceder al menu de vehículos", vector3(playerX, playerY, playerZ))
							if Menu.hidden then
								local posicion = actionData["position"].x, actionData["position"].y, actionData["position"].z
								esx.ShowFloatingHelpNotification("Pulsa [E] para acceder al menu de vehículos", actionData["position"] - vector3(0.0, 0.0, 0.0-1.9))
