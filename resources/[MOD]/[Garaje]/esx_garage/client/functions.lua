...
SpawnLocalVehicle = function(data)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
        esx.ShowNotification('Hay un obstaculo en la salida')
...
SpawnVehicle = function(data,recuperar)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
        esx.ShowNotification('Despeje la zona para poder sacar el vehículo')
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                esx.ShowNotification('Tu vehículo ya está fuera, búscalo !!')
    ...
    if recuperar then
        esx.ShowNotification('Has pagado 200$ para recuperar su vehículo!')
...
SaveInGarage = function(garage)
    ...
    esx.ShowNotification('Guardaste tu vehículo en el garaje')
    Citizen.Wait(500)
...
DrawScriptMarker = function(markerData)
    DrawMarker(1, markerData["pos"],0,0,0,0,0,0,1.4,1.4, 0.2, 255, 0, 0, 0.05,0, 0,0,0,0,0,0)
    end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        esx.ShowNotification('Este modelo de coche no existe en la ciudad')
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Buscando tu vehículo " .. GetDisplayNameFromVehicleModel(model) .. "...", 255, 255, 255, 150)
	end
end