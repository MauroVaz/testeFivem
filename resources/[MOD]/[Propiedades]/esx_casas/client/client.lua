...
Citizen.CreateThread(function()
    while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(5) end
    while ESX.GetPlayerData().job == nil do Wait(5) end
    TriggerServerEvent('esx_casas:getOwned')
    while OwnedHouse == nil do Wait(5) end
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data, menu)
                                    if data.current.value == 'enter' then
                                        TriggerServerEvent('esx_casas:enterHouse', k)
                                        menu.close()
                                    ...
                                    elseif data.current.value == 'sell' then
                                        ...
                                        function(data2, menu2)
                                            if data2.current.value == 'yes' then
                                                TriggerServerEvent('esx_casas:sellHouse')
                                                ESX.UI.Menu.CloseAll()
                                                Wait(WaitTime)
                                            else
                                                menu2.close()
                            ...
                            if not AvailableHouses[k] and OwnedHouse.houseId == 0 then
                                ...
                                function(data, menu)
                                    if data.current.value == 'yes' then
                                        TriggerServerEvent('esx_casas:buyHouse', k)
                                        ESX.UI.Menu.CloseAll()
                                        Wait(WaitTime)
                                    else
                                        menu.close()
                ...
                if OwnedHouse.houseId == k then
                    ...
                    if found then
                        while #(GetEntityCoords(PlayerPedId()) - coords) <= 45.0 and IsPedInAnyVehicle(PlayerPedId(), false) and Config.EnableGarage do
                            s = 0
                            DrawMarker(1, coords-vector3(0.0, 0.0, 0.5), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.2), 255, 255, 255, 255, false, false, 2, false, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if dist <= 50.0 then
            s = 0
            DrawMarker(1, Config.Furnituring['enter'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.5, 1.5, 0.4), 255, 255, 255, 255, false, false, 2, false, false, false)
            if dist <= 1.5 then
                    ...
                    while true do
                        ...
                        elseif IsControlJustReleased(0, 191) then
                            ...
                            function(data, menu)
                                menu.close()
                                if data.current.value == 'yes' then
                                    TriggerServerEvent('esx_casas:buy_furniture', currentcategory, current)
                                    DoScreenFadeOut(250)
                                    Wait(1500)
                                    DoScreenFadeIn(500)
                                end
...
RegisterNetEvent('esx_casas:spawnHouse')
AddEventHandler('esx_casas:spawnHouse', function(coords, furniture)
    ...
    TriggerServerEvent('esx_casas:setInstanceCoords', exit, coords, prop, OwnedHouse['furniture'])
    DoScreenFadeOut(750)
    ...
    while in_house do
        ...
        DrawMarker(1, exit, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.5, 1.5, 0.1), 255, 255, 255, 255, false, false, 2, false, false, false)
        DrawMarker(1, storage, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.5, 1.5, 0.1), 255, 255, 255, 255, false, false, 2, false, false, false)
            ...
            if IsControlJustReleased(0, 38) and Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
                    ...
                    'default', GetCurrentResourceName(), 'storage',
                {
                    ...
                    elements = {
                        ...
                        {label = Strings['Black_Money'], value = 'm'}
                    },
                },
                function(data, menu)
                    ...
                    elseif data.current.value == 'm' then
                        moneyStorage(OwnedHouse.houseId)
                    end
                end, 
                    function(data, menu)
                    menu.close()
            ...
            if IsControlJustReleased(0, 38) then
                ...
                function(data, menu)
                    if data.current.value == 'exit' then
                        ESX.TriggerServerCallback('esx_casas:hasGuests', function(has)
                            if not has then
                                ESX.UI.Menu.CloseAll()
                                TriggerServerEvent('esx_casas:deleteInstance')
                                Wait(3500)
                                in_house = false
                                return
                            else
                        ...
                        ESX.TriggerServerCallback('esx_casas:hasGuests', function(has)
                            if not has then
                                ...
                                function(data2, menu2)
                                    ...
                                    TriggerServerEvent('esx_casas:furnish', OwnedHouse, furniture)
                                    menu2.close()
                                    editing = false
                                end, 
                                    function(data2, menu2)
                    ...
                    elseif data.current.value == 'accept' then
                        ...
                        function(data2, menu2)
                            if Knockings[data2.current.value] then
                                TriggerServerEvent('esx_casas:letIn', data2.current.value, storage)
                            end
                            menu2.close()
                        end, 
                            function(data2, menu2)
...
RegisterNetEvent('esx_casas:leaveHouse')
AddEventHandler('esx_casas:leaveHouse', function(house)
    DoScreenFadeOut(750)
    while not IsScreenFadedOut() do Wait(0) end
...
RegisterNetEvent('esx_casas:knockAccept')
AddEventHandler('esx_casas:knockAccept', function(coords, house, storage, spawnpos, furniture, host)
    ...
    while true do
        ...
        if timer <= GetGameTimer() then
            ...
            ESX.TriggerServerCallback('esx_casas:hostOnline', function(online)
                if not online then
                    delete = true
                end
            end, host)
        ...
        DrawMarker(1, coords, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 255, false, false, 2, false, false, false)
        DrawMarker(1, storage, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 255, false, false, 2, false, false, false)
        if Vdist2(GetEntityCoords(PlayerPedId()), coords) <= 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                DeleteObject(prop)
                DoScreenFadeIn(1500)
                TriggerServerEvent('esx_casas:leaveHouse', house)
                return
            end
        end
        if Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
...
RegisterNetEvent('esx_casas:reloadHouses')
AddEventHandler('esx_casas:reloadHouses', function()
    while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(0) end
    while ESX.GetPlayerData().job == nil do Wait(0) end
    TriggerServerEvent('esx_casas:getOwned')
end)

RegisterNetEvent('esx_casas:knockedDoor')
AddEventHandler('esx_casas:knockedDoor', function(src)
...
RegisterNetEvent('esx_casas:removeDoorKnock')
AddEventHandler('esx_casas:removeDoorKnock', function(src)
    local newTable = {}
    for k, v in pairs(Knockings) do
        if v.value ~= src then
...
RegisterNetEvent('esx_casas:setHouse')
AddEventHandler('esx_casas:setHouse', function(house, purchasedHouses)
    OwnedHouse = house
...
HelpText = function(msg, coords)
    if not coords or not Config.Use3DText then
        ...
        ESX.Game.Utils.DrawText3D(vector3(coords.x,coords.y,coords.z+1), string.gsub(msg, "~INPUT_CONTEXT~", "~r~[~w~E~r~]~w~"), 0.35, 0)
    end
end
...
RegisterCommand('casas', function()
    Config.AddHouseBlips = not Config.AddHouseBlips
    TriggerServerEvent('esx_casas:getOwned')
end)