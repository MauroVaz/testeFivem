...
function OpenShopMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			elseif data2.current.value == 'coronas' then
				if Config.EnablePlayerManagement then
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehicleCoronas', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
				...
				ESX.ShowFloatingHelpNotification("Pulsa [E] para acceder a la tienda", vector3(v.Pos.x, v.Pos.y, v.Pos.z + 1.5))
            end
        end
        Citizen.Wait(_msec)
    end
