			...
			if not menuactive and distance <= 20.0 then
				...
				if distance <= 2.0 then
					...
					if IsControlJustPressed(0,38) then
						empresaAtual = k
						TriggerServerEvent("esx_stores:getData",empresaAtual) 
					end
				end
			end
			...
			for _,mark in pairs(v.sell_blip_coords) do
				...
				if distance <= 20.0 then
					...
					if distance <= 2.0 then
						...
						if IsControlJustPressed(0,38) then
							empresaAtual = k
							TriggerServerEvent("esx_stores:openMarket",k) 
						end
					end
				end
			end
			...
			if distance <= 20.0 then
				...
				if distance <= 2.0 then
					if job_data[k] == nil then
						...
						if IsControlJustPressed(0,38) then
							TriggerServerEvent('esx_stores:getJob',k)
						end
					else
						...
						if IsControlJustPressed(0,38) then
							...
							empresaAtual = k
							TriggerServerEvent('esx_stores:startJob',k,job_data[k].id)
						end
					end
				else
					job_data[k] = nil
...
RegisterNetEvent('esx_stores:getJob')
AddEventHandler('esx_stores:getJob', function(k,data)
	job_data[k] = data
end)

RegisterNetEvent('esx_stores:open')
AddEventHandler('esx_stores:open', function(dados,update,isMarket)
	-- Abre NUI
	SendNUIMessage({ 
		showmenu = true,
		update = update,
...
RegisterNUICallback('startJob', function(data, cb)
	...
	TriggerServerEvent('esx_stores:startContract',empresaAtual,data.item_id)
end)

RegisterNUICallback('createJob', function(data, cb)
	TriggerServerEvent('esx_stores:createJob',empresaAtual,data)
end)

RegisterNUICallback('buyUpgrade', function(data, cb)
	TriggerServerEvent('esx_stores:buyUpgrade',empresaAtual,data)
end)
...
RegisterNUICallback('deleteJob', function(data, cb)
	if cooldown1 == nil then
		...
		TriggerServerEvent('esx_stores:deleteJob',empresaAtual,data)
...
RegisterNUICallback('depositMoney', function(data, cb)
	TriggerServerEvent('esx_stores:depositMoney',empresaAtual,data)
end)
...
RegisterNUICallback('withdrawMoney', function(data, cb)
	if cooldown2 == nil then
		...
		TriggerServerEvent('esx_stores:withdrawMoney',empresaAtual)
...
RegisterNUICallback('sellMarket', function(data, cb)
	if cooldown3 == nil then
		...
		closeUI()
		TriggerServerEvent('esx_stores:sellMarket',key)
...
RegisterNUICallback('buyItem', function(data, cb)
	TriggerServerEvent('esx_stores:buyItem',empresaAtual,data)
end)
...
RegisterNetEvent('esx_stores:startContract')
AddEventHandler('esx_stores:startContract', function(item,truck_level,query_delivery)
	local key = empresaAtual
	job_data[key] = nil
	...
	while truck do
		...
		elseif fase_coleta == 3 then
			...
			if distance <= 50 and veh == truck then
				...
				if distance <= 4 then
					...
					truck_blip = nil
					route_blip = nil
					TriggerServerEvent("esx_stores:finishContract",key,item,truck_level,distance_traveled,query_delivery)
					return
				end
			end
		end
		...
		if not IsEntityAVehicle(truck) then
			...
			truck_blip = nil
			route_blip = nil
			TriggerServerEvent("esx_stores:failed",query_delivery)
			return
		end
...
function DeletarObjeto()
    ...
    if DoesEntityExist(object) then
        TriggerServerEvent("esx_stores:trydeleteobj",ObjToNet(object))
        object = nil
    end
end

RegisterNetEvent("esx_stores:syncdeleteobj")
AddEventHandler("esx_stores:syncdeleteobj",function(index)
	...
	while true do
		if truck then
			...
			if IsControlJustPressed(0,Config.keyToUnlockTruck) then
				TriggerServerEvent("esx_stores:vehicleLock")
			end
		end
		Citizen.Wait(timer)
	end
...
RegisterNetEvent('esx_stores:vehicleClientLock')
AddEventHandler('esx_stores:vehicleClientLock', function()
	local v = truck
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		TriggerEvent("esx_sound:source",'lock',0.5)
		if lock == 1 then
