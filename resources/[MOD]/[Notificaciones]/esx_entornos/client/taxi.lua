...
AddEventHandler('playerSpawned', function(spawn)
  TriggerServerEvent('AGM_taxi:getJob')
end)
...
RegisterNetEvent('AGM_taxi:setJob')
AddEventHandler('AGM_taxi:setJob',function(theJob)
    job = theJob
end)
...
RegisterNetEvent('AGM_taxi:sendNotify')
AddEventHandler('AGM_taxi:sendNotify', function( msg, location, pos, IdPlayer )
  if job == 'taxi' then 
    ...
    TriggerEvent('AGM_Notify:SendNotification',{text = "LLAMADA TAXI  </br> Motivo: "..msg.."  </br> Ubicación:  "..location.." </br>  Distancia: "..dist.." Km </br> <p> </p><b style='color:#3efe00'>[E Aceptar] </b> <b style='color:#fe0000'>[Q Denegar] </b>", 
    timeout = 15000,
      type = "warning",
      progressBar = true,
      layout     = "centerRight",
...
RegisterNetEvent('AGM_taxi:setBlip')
AddEventHandler('AGM_taxi:setBlip', function(x, y, z)
    ...
    while taxi_llamada do
      if IsControlJustReleased(1, 38) then
        ...
        taxi_llamada = false
        TriggerEvent('AGM_taxi:remblip', blip)
...
RegisterNetEvent('AGM_taxi:remblip')
AddEventHandler('AGM_taxi:remblip', function(blip)
  local newblip = blip
  Wait(120)
  RemoveBlip(newblip)
end)
...
RegisterCommand('taxi', function(source, args)
  ...
  if args[1] == nil then
    ...
    TriggerServerEvent('AGM_taxi:sendNotify', msg, location, pos, IdPlayer)
    TriggerServerEvent('AGM_taxi:alert', message, x, y, z)
  end
end)