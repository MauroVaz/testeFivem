...
RegisterNetEvent("AGM_Notify:SendNotification")
AddEventHandler("AGM_Notify:SendNotification", function(options)
    SendNotification(options)
end)

RegisterNetEvent("AGM_Notify:SetQueueMax")
AddEventHandler("AGM_Notify:SetQueueMax", function(queue, max)
    SetQueueMax(queue, max)
end)

--[[RegisterNetEvent("chatMessage")
AddEventHandler("chatMessage", function(author, color, text)
    TriggerEvent("AGM_Notify:SendNotification", {text = "<span style='font-weight: 900'>" .. text .. "</span>",
        layout = "centerLeft",
        timeout = 2000,
        progressBar = false,
        type = "error",
