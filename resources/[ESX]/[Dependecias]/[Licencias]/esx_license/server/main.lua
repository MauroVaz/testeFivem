...
function AddLicense(target, type, cb)
	...
	if xPlayer then
		MySQL.Async.fetchAll('SELECT owner FROM user_licenses WHERE owner=@owner AND type=@type',
			...
			function(res)
				if res[1] ~= nil then
					xPlayer.showNotification('Te estan intentando poner una licencia que ya tienes')
				else
...
function GetLicenses(target, cb)
	...
	MySQL.Async.fetchAll('SELECT type FROM user_licenses WHERE owner = @owner', {
		...
		local licenses, asyncTasanon = {}, {}

		for i=1, #result, 1 do
			local scope = function(type)
				table.insert(asyncTasanon, function(cb)
					MySQL.Async.fetchAll('SELECT label FROM licenses WHERE type = @type', {
						['@type'] = type
					}, function(result2)
		...
		Async.parallel(asyncTasanon, function(results)
			cb(licenses)
		end)
	end)
end
