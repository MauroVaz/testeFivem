...
local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
	AddTextEntry('esxHelpNotification' , msg)
	SetFloatingHelpTextWorldPosition(1  , vector3(playerX, playerY, playerZ+1))
						...
						ESX.UI.Menu.Open('default' , GetCurrentResourceName() , 'give_item_to' , {
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										...
										ESX.UI.Menu.Open('dialog' , GetCurrentResourceName() , 'inventory_item_count_give' , {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												...
												exports['pNotify']:DoCustomHudText('cajaroja' , ('Cantidad invalida') , 5000)
											end
									...
									exports['pNotify']:DoCustomHudText('cajaroja' , ('¡No puedes darle nada a alguien en un vehículo!') , 5000)
								end
							else
								exports['pNotify']:DoCustomHudText('cajaroja' , ('¡No hay jugadores cerca!') , 5000)
								menu2.close()
							end
							...
							ESX.UI.Menu.Open('dialog' , GetCurrentResourceName() , 'inventory_item_count_give' , {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										...
										exports['pNotify']:DoCustomHudText('cajaroja' , ('¡No tienes munición!') , 5000)
									end
								else
									exports['pNotify']:DoCustomHudText('cajaroja' , ('Cantidad invalida') , 5000)
								end
