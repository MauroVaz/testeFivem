ESX.Trace                   = function(msg)
	if Config.EnableDebug then
		print(('[sapo-base] [^2TRACE^7] %s^7'):format(msg))
	end
end
...
ESX.RegisterCommand         = function(name , group , cb , allowConsole , suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[sapo-base] [^3WARNING^7] An command "%s" is already registered, overriding command'):format(name))
	...
	RegisterCommand(name , function(playerId , args , rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[sapo-base] [^3WARNING^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[sapo-base] [^3WARNING^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false , args , function(msg)
					if playerId == 0 then
						print(('[sapo-base] [^3WARNING^7] %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback   = function(name , requestId , source , cb , ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[sapo-base] [^3WARNING^7] Server callback "%s" does not exist. Make sure that the server sided file really is loading, an error in that file might cause it to not load.'):format(name))
	end
end
...
ESX.SavePlayers             = function(cb)
	...
	Async.parallelLimit(asyncTasks , 8 , function(results)
		print(('[sapo-base] [^2INFO^7] Saved %s player(s)'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
