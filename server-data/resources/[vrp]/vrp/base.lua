...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookconnect = "https://discordapp.com/api/webhooks/678295183154217015/kR0EBKtsly8oA6Luv7BBnK94ow2pHIf9l1vn69ypnzdgJIxrNd7SX-L14Nq63WWSFNM7"
local webhookdisconnect = "https://discordapp.com/api/webhooks/678295245158612996/GSVBzIdmYxkIDXTC0X-3rzJJn-Yd9oT3XMO7pn1XhyrBBsFa-XtHjtlPe95zAW9hbqeJ"
...
AddEventHandler("queue:playerConnecting",function(source,ids,name,setKickReason,deferrals)
	...
	if ids ~= nil and #ids > 0 then
		deferrals.update("[FURYPVP] Checando identidades...")
		local user_id = vRP.getUserIdByIdentifiers(ids)
		if user_id then
			deferrals.update("[FURYPVP] Checando banimentos...")
			if not vRP.isBanned(user_id) then
				deferrals.update("[FURYPVP] Checando whitelist...")
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("[FURYPVP] Carregando banco de dados...")
